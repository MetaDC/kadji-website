(function ($) {
  ...
  var galleryThumbs;
  if (typeof Swiper !== 'undefined' && $('.product-details__gallery-thumb').length) {
    galleryThumbs = new Swiper('.product-details__gallery-thumb', {
      slidesPerView: 4,
      spaceBetween: 30,
      speed: 1400,
  ...
  var galleryTop;
  if (typeof Swiper !== 'undefined' && $('.product-details__gallery-top').length) {
    galleryTop = new Swiper('.product-details__gallery-top', {
      observer: true,
      observeParents: true,
      loop: true,
      speed: 1400,
  ...
  $(window).on("load", function () {
    ...
    // Global YouTube iframe lazy loader (bfcache compatible)
    let globalIframeObserver;
    function setupGlobalIframeObserver() {
        ...
        globalIframeObserver = new IntersectionObserver((entries) => {
          entries.forEach((entry) => {
            if (entry.isIntersecting) {
              ...
              if (src && (!iframe.src || iframe.src === 'about:blank' || iframe.src.includes('about:blank'))) {
                iframe.src = src;
              }
              globalIframeObserver.unobserve(iframe);
            }
          });
        }, { rootMargin: '200px' });
        lazyIframes.forEach((iframe) => globalIframeObserver.observe(iframe));
      }
    }

    function unloadGlobalIframeSources() {
      document.querySelectorAll('iframe[data-src]').forEach((iframe) => {
        iframe.removeAttribute('src');
      });
    }

    setupGlobalIframeObserver();

    window.addEventListener("pagehide", function () {
      unloadGlobalIframeSources();
    });
